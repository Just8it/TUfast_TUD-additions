...
export function inferNodeType(url: string): 'course' | 'folder' | 'file' {
...
export function readBestLinkTitle(
  ...
  return title ? cleanLinkTitle(title) : ''
}
...
export function extractCourseNodeLinks(root: Document | HTMLElement, courseUrl: string): ExtractedCourseNodeLink[] {
  ...
  const html =
    root instanceof Document ? root.documentElement.outerHTML : root instanceof HTMLElement ? root.outerHTML : null
  if (html) {
