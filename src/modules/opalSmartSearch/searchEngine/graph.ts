...
function extractRepositoryCourseId(value: string): string | null {
  return value.match(/(\/opal\/[^/]+\/RepositoryEntry\/\d+)/i)?.[1] ?? null
}

function extractFolderResourceCourseId(value: string): string | null {
